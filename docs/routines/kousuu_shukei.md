...
- スケジュール: 毎月2日 09:00（JST）が理想。ただし Routines 画面のプリセットに
...
この Routine は毎日起動する設定になっている場合があります。bash の date で本日のJST日付を確認してください。

日が2日でない場合は、以降の工程を一切行わず、「本日は対象日（毎月2日）ではないため実行しません」と1行だけ報告して終了します。Box への保存も、カレンダーの取得も行いません。
