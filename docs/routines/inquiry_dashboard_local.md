...
正本は マスターナレッジ の `claude/20260916_横断_ダッシュボードBox配置_Desktopタスク指示文_v1_8.md` である。
記述が食い違った場合は正本を優先する。
...
コピー直後に、config/revenue.json が既に存在する場合は削除する。

rm -f config/revenue.json

これは、スキルの同期コピー（SKILL_DIR）自体に古いconfig/revenue.jsonが含まれていた場合に備えるためである。含まれていると、次の手順で上書き層の既定値が「既にあります。触りません」として素通りされ、リポジトリ側で最新化した年商レンジの参照データが反映されない。
...
続けて config/revenue.json の list の件数も数え、控える。0件だった場合は、参照データが反映されていない可能性があるため、その旨を報告へ明記する。
...
- config/revenue.json の list の件数（配置直後）。0件だった場合はその旨
...
## 確認結果（2026/09/16 18時台、Coworkから実機を確認）

正本 v1.8 の第9.5章が仮説として挙げた経路について、スキルの実体を直接確認した結果を記録する。

| 確認項目 | 結果 |
|---|---|
| スキルの同期コピーの config に revenue.json が含まれるか | 含まれない。config の中身は advice_history.js.txt、bc.json、company_overrides.json、contents.js.txt、exclusions.json、industry_overrides.json、inquiry.json、market.json、playbook.json、rf2_records.json のみ。したがって第9.5章の仮説は成り立たない |
| 7社分を追加したコミット 56737fd の時刻 | 2026-09-16 17:18:39（日本時間）。17:54 開始の実行より前であり、実行時点でリポジトリ側にデータはあった |
| 18時台の再実行 | config/revenue.json が7社入りで配置され、分析版（377440バイト）にデータが埋め込まれた状態で出力フォルダへ配置された |
| 台帳CSVの社名と参照データの社名 | 完全一致（大和ハウス工業株式会社、株式会社デンソー、九州電力株式会社、BIPROGY株式会社ほか）。照合関数 revNorm で突き合う形になっている |

現象は解消しているが、v1.8 の対策が効いたのか別の要因が消えたのかは区別できていない。
画面上で行が 1000億円以上 と描画されることは未確認である。

未解決として残る構造上の点を1件記録する。判断は未了であり、正本へは反映していない。

リポジトリの最新化（git fetch・checkout・pull）が手順3にあり、上書き層を当てる手順1と生成の手順2より後になっている。
手順1の時点でブランチが別のものであったりコミットが古かったりすると、その回は古い差分と古い参照データで生成され、
リポジトリの変更が1回遅れて反映される。手順0の末尾へ移す案を提示済みである。
